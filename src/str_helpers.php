<?php
    ...
    function str_title_from_array($ar, $key = null){
        if(!is_array($ar)) return str_title_case($ar);
        if(isset($ar['title'])) return $ar['title'];
        return str_title_case($key);
    }
    
    function str_key_from_array($ar, $key = null){
        if(is_array($ar)) return $key;
        if(is_numeric($key)) return $ar;
        return $key;
    }
    ...
    function str_plurals($str, $join = '.'){
        if(empty($str)) return '';
        if(strpos($str, $join) === false) return str_plural($str);
